...
const Devocionales = () => {
    ...
    const [userExists, setUserExists] = useState(false); // Nuevo estado
    const userLocal = JSON.parse(user)
    useEffect(() => {
        fetchData();
        setUserExists(user !== null); // Actualizar el estado cuando el usuario no es nulo
    }, [user]);
    ...
    return (
        <View style={styles.container}>
            ...
            {/* Agregar condicional para renderizar si userExists es verdadero */}
            {userExists && (
                <Text style={styles.userText}>¡El usuario existe y el componente se renderiza! {userLocal.rol}</Text>
            )}
